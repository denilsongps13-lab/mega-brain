...
def install():
    ...
    if target is None:
        ...
        ui.destroy()
        if not directory:
            print('Instalacao cancelada.'); return 1
        target = pathlib.Path(directory)
    if not (target/'.claude/settings.json').is_file():
        print('Pasta incorreta: .claude/settings.json ausente.'); return 1
    payload = json.loads(base64.b64decode(PAYLOAD))
    decoded = {}
    for relative, item in payload.items():
        data = base64.b64decode(item['data'])
        if hashlib.sha256(data).hexdigest() != item['sha256']:
            print('Pacote corrompido. Nenhum arquivo alterado.'); return 1
    ...
    for relative, data in decoded.items():
        ...
        if current != previous[relative]:
            print('Arquivo alterado por outro processo. Instalacao interrompida; backup disponivel.'); return 1
    ...
    return subprocess.call([sys.executable, str(target/'scripts/start_mega_brain.py')], cwd=target)
