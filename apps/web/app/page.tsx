...
import { type Agent, type Conversation, type Doc, type Job, type Message, type Provider, type Source, client, labels, terminal, watchJob } from '@/lib/api';
...
export default function Home() {
  ...
  useEffect(() => { if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js').catch(() => {}); return () => stop.current?.(); }, []);
  ...
  async function login(e: React.FormEvent) {
    ...
    try { const a = client(inputToken); const s = await a<Record<string,unknown>>('/api/status'); const c = await a<Conversation[]>('/api/chat/conversations'); setStatus(s); setConversations(c); setToken(inputToken); setInputToken(''); setConnection('available'); }
