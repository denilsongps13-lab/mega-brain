...
async function login(page: import('@playwright/test').Page) {
  await page.goto('/');
  await page.getByLabel('Token de acesso da instalação').fill(token);
  await page.getByRole('button', { name: 'Acessar meu cérebro' }).click();
...
test('desktop: real API upload, RAG, provider failure and persistent history', async ({ page }) => {
  ...
  await page.getByLabel('Enviar documento').setInputFiles({
    name: 'aurora-e2e.txt',
    mimeType: 'text/plain',
    buffer: Buffer.from('O projeto Aurora usa baterias solares em comunidades isoladas. '.repeat(15)),
  });
  ...
  await page.getByLabel('Pesquisar memória').fill('Aurora baterias');
  ...
  await page.getByLabel('Mensagem', { exact: true }).fill('Explique o projeto Aurora');
  await page.getByRole('button', { name: 'Enviar mensagem' }).click();
  await expect(page.getByText('Explique o projeto Aurora', { exact: true }).first()).toBeVisible();
  await expect(page.locator('.error-banner[role="alert"]')).toContainText('Falha no engine', {
    timeout: 20000,
  });
